classdef KDLOR_I < Algorithm
    ...
    methods
        ...
        function obj = KDLOR_I(kernel, opt)
        ...
        function [model_information] = runAlgorithm(obj,train, test, parameters)
                ...
                obj.numPatterns = size(train.patterns',2)+ size(test.patterns',2);
                numClasses = length(unique(train.targets'));
                obj.ppc = zeros(numClasses,1);
                
                for currentClass = 1:numClasses,
                  obj.ppc(currentClass,:)=size(train.targets(train.targets == currentClass,:),1);
                end
                for currentClass = 1:numClasses,
                  obj.ppc(currentClass,:)=obj.ppc(currentClass,:)+size(test.targets(test.targets == currentClass,:),1);
                end
        ...
        function [model]= train( obj,trainPatterns, trainTargets , parameters)
                ...
                meanClassesInputSpace = zeros(dim,numClasses);
                ...
                for currentClass = 1:numClasses,
                  meanClassesInputSpace(:,currentClass) = mean(trainPatterns(:,( trainTargets == currentClass ))');
                end
                kernelMatrixMean = computeKernelMatrix(meanClassesInputSpace,meanClassesInputSpace,obj.kernelType,kernelParam);
                
                relevance = zeros(numClasses,1);
                for currentClass = 1:numClasses,
                    for jClass = 1:numClasses,
                        if (currentClass ~= jClass) 
                            relevance(currentClass) = relevance(currentClass) + (1/(2*(1-kernelMatrixMean(currentClass,jClass))));                        
                        end
                    end
                end
                ...
                for currentClass = 1:numClasses,
                  ...
                  H = H + ((1-((obj.ppc(currentClass) -1)/obj.numPatterns))*(relevance(currentClass))).*(kernelMatrix(:,( trainTargets == currentClass ))*(eye(N(1,currentClass),N(1,currentClass))-ones(N(1,currentClass),N(1,currentClass))/sum( trainTargets == currentClass ))*kernelMatrix(:,( trainTargets == currentClass ))');
                end
