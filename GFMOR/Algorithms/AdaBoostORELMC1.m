classdef AdaBoostORELMC1 < Algorithm
    ...
    methods
        ...
        function obj = AdaBoostORELMC1(opt)
            obj.name = 'AdaBoost and ELM Ordinal Regression (C1 Cost)';
        ...
        function [model]= train( obj,train,parameters)
                    ...
                    for j = 1:nOfEnsembles,
                        ...
                        errorELM = sum(indicatorFunction.*weights')/sum(weights');
                        ...
                        %C1
                        % Update weights
                        %weights = weights.* (exp(alpha(j,:).*indicatorFunction))';     
                        weights = cost'.*weights.* (exp(alpha(j,:).*indicatorFunction))';     
                    ...
                    model.algorithm = 'AdaBoost.OR(ELM)-C1';
